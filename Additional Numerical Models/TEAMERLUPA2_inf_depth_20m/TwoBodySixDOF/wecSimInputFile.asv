...
% waves.period = 10;                    % [s]  4.47s to 14.53s Examples of scaled periods used for LUPA (Based on scaled experimental periods tested
...
pto(1).damping = 0;                                     % [N/(m/s)] PTO Damping. Typical values fall between 0-17e6 N/(m/s); scaled 20x from experimental values                     
pto(1).location = [0 0 0];                              % [m] PTO Location 

% ~~Optional linear damping loss~~
% Included in lab-scale numerical model to account for friction loss. 
% Even scaled up 20x this loss is neglegible for a utility-scaled WEC. 
% 20x scaled damping loss can be set to -626099 [N/m/s] based on
% experimentally found values. Value is negative due to electromechanical
% loss. 
ptoDampingLoss = 0;                                     % [N/m/s] Linear damping loss

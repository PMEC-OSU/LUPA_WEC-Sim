...
deSpike=struct();
deSpike.negThresh = 1e-2; % the threshold below which negative damping will be removed
deSpike.N = 7; % will loop the despiking procedure N time before filtering
...
deSpike.Threshold.B = 2e-2; % damping
deSpike.Threshold.A = 1e-1; % added mass
deSpike.Threshold.ExRe = 5e-1; % real part excitation
deSpike.Threshold.ExIm = 5e-1; % imag part excitation

% minimum peak prominence, applied to 'MinPeakProminence' argument of findpeaks 
deSpike.Prominence.B = 2e-2;  
deSpike.Prominence.A = 1e-1; 
deSpike.Prominence.ExRe = 5e-1;  
deSpike.Prominence.ExIm = 5e-1; 

% minimum peak distance, applied to 'MinPeakDistance' argument of findpeaks 
deSpike.MinPeakDistance.A = 1;
deSpike.MinPeakDistance.B = 1;
deSpike.MinPeakDistance.ExRe = 0.05;
deSpike.MinPeakDistance.ExIm = 0.05;
